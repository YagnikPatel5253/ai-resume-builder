    ...
    const GenerateSummeryFromAI = async () => {
        ...
        try {
            ...
            if (Array.isArray(parsedResult)) {
                ...
                console.error('AI response is not an array:', parsedResult);
                setAiGenerateSummeryList([]); // fallback to an empty array
            }
        } catch (error) {
            console.error('Error generating summary from AI:', error);
            setAiGenerateSummeryList([]); // fallback to an empty array on error
        }
        setLoading(false);
    }
        ...
        <div>
            <div className='p-5 shadow-lg rounded-lg border-t-primary border-t-4 mt-10'>
                ...
                <form className='mt-7' onSubmit={onSave}>
                    ...
                    <Textarea className="mt-5 h-48" required
                        value={summery}
            ...
            {Array.isArray(aiGeneratedSummeryList) && aiGeneratedSummeryList.length > 0 && (
                <div className='my-5'>
                    ...
                    {aiGeneratedSummeryList.map((item, index) => (
                        <div key={index} onClick={() => setSummery(item?.summary)}
